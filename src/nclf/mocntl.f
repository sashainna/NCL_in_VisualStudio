...
C
      ...
      integer*2 ksn(4),lvgt3,ksc(60)
      integer*4 topkey,botkey,nptkey,nn
      integer*4 isv2(2), iept(2)
      real*4 lvsav
      real*8 zbot,ztop,stkoff,maxgap,lastmx,lastmy,lastmz
      real*8 endmx,endmy,endmz,cnvmm,temp,fw(3),pnt(3)
c      real*8 endmx,endmy,endmz,cnvmm
c
      equivalence (asn,ksn)
      equivalence (lastmx, sc(171)), (lastmy, sc(172))
      equivalence (lastmz, sc(173)), (ksc,sc)
      equivalence (isc19 , sc(19))

      logical lgfa,lv92,lv93,lv97, g3more
...
c************ pmill ***************
48    if(mtyp.ne.1051) goto 50
c      if(mtyp.ne.703) goto 50
      call pmill
      if (ifl(347).ge.1. and. ifl(2).lt.1 .and. (ifl(42).eq.0 .or.
     1    ifl(372) .eq. 1)) then
         ifl(347) = ifl(347) +1
...
c      
c....Included by Sasha from the 101 version to support vpmill  July 19, 2018       
c

c...vp 4/3/98 if GO to 3 SF returns FWD sense reversed try again
c...with reversed DS and CS
c..... Eduard 5/25/1999. Do the same trick if the error flag is 256.
c
      if ((lvgt3.eq.1 .or. lvgt3.eq.2) .and. g3more) then
        ...
        if (lv93) then
          ...
          ksc(39) = 3
c
c... Swap UY_ds & UY_cs[0] for NCLX geometries. jingrong 01/09/2000.
c
          call ncl_dscs_swap
c
c......Swap thicks
...
c
c..... Eduard 8/27/1999. Try moving the tool a little along the 
...
c
      if (lvgt3.eq.3 .and. g3more) then
        ...
        sc(15) = hgo(15)
c
c... Restore UY_ds & UY_cs[0] for NCLX geometries. jingrong 01/09/2000.
c 
...
c      
c....End of Included by Sasha from the 101 version to support vpmill  July 19, 2018    
c
...
c         The block commented out below by Sasha on May 03, 2019, was not necessary to prevent flips 
