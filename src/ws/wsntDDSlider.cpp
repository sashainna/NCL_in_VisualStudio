...
#if UU_COMP == UU_WIN2K
/************************************************************************
**		FILE NAME: wsntDDSlider.cpp
**
**	 Description - Functions and implementations for
**		CNCLDDSlider class (NCL Slider)
**		with Drag &Drop
**	 CONTAINS:
**		member function of CNCLDDSlider
**
**    COPYRIGHT 2015 (c) NCCS.  All Rights Reserved.
**    MODULE NAME AND RELEASE LEVEL 
**			wsntDDSlider.cpp , 25.2
...
IMPLEMENT_DYNAMIC(CNCLDDSlider, CNCLSliderCtrl)

/***********************************************************************
c
c   SUBROUTINE:  CNCLDDSlider
c
...
c***********************************************************************
*/
CNCLDDSlider::CNCLDDSlider(int type) 
	:	m_TargetDrop(NULL),
		m_TimerID(0)
{  
#if (_MFC_VER < 0x0250)
...
BEGIN_MESSAGE_MAP(CNCLDDSlider, CNCLSliderCtrl)
	//{{AFX_MSG_MAP(CNCLDDSlider)
	ON_WM_LBUTTONUP()
	ON_WM_LBUTTONDOWN()
	ON_WM_RBUTTONUP()
	ON_WM_MOUSEMOVE()
...
**
**********************************************************************/
int CNCLDDSlider::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if (CNCLSliderCtrl::OnCreate(lpCreateStruct) == -1)
		return -1;
	if (m_prop_dlg==NULL)
	{
...
c
**********************************************************************/
void CNCLDDSlider::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_StartPoint = 	point;
	m_TimerID = SetTimer(1, 100, NULL);	
	CNCLSliderCtrl::OnLButtonDown(nFlags, point);
...
c
**********************************************************************/
void CNCLDDSlider::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_StartPoint.x = -100;
	m_StartPoint.y = -100;
	...
	CNCLSliderCtrl::OnLButtonUp(nFlags, point);
/*
......draw the select box and select this item
*/
	ClientToScreen(&point);
...
c
**********************************************************************/
void CNCLDDSlider::OnRButtonUp(UINT nFlags, CPoint point) 
{
	if (m_parent==NULL)
		return;
	CMenu pmenu;
...
*/
	...
	CNCLSliderCtrl::OnRButtonUp(nFlags, point);
}
void CNCLDDSlider::SetProperty(CNCLFormProp *prop_dlg)
{
	DWORD dwStyle = GetStyle();
	if ((dwStyle & TBS_VERT )&&(prop_dlg->m_justified==0))
	{
		ModifyStyle(TBS_VERT, TBS_HORZ, 1);
	}
	else if ((!(dwStyle & TBS_VERT)) && (prop_dlg->m_justified==1))
	{
		ModifyStyle(TBS_HORZ, TBS_VERT, 1);
	}
	RECT rectClient;
	GetWindowRect(&rectClient);	
	m_parent->ScreenToClient(&rectClient);
...
/*
	if (dwStyle & TBS_VERT )
	{
		if (rectClient.right-rectClient.left>20)
			rectClient.right = rectClient.left + 20;
	}
	else
	{
		if (rectClient.bottom-rectClient.top>20)
...
**
**********************************************************************/
void CNCLDDSlider::OnTimer(UINT_PTR nIDEvent) 
{
	if (nIDEvent == 1)
	{
		POINT pt;
	...
	CNCLSliderCtrl::OnTimer(nIDEvent);
}
/***********************************************************************
**
...
**
**********************************************************************/
void CNCLDDSlider::OnSize(UINT nType, int cx, int cy) 
{
	CNCLSliderCtrl::OnSize(nType, cx, cy);
...
**
**********************************************************************/
void CNCLDDSlider::OnMove(int x, int y)
{
	CNCLSliderCtrl::OnMove(x, y);
	CRect rectClient;
	GetWindowRect(&rectClient);	
	m_parent->ScreenToClient(&rectClient);
	if (m_prop_dlg==NULL)
...
c
**********************************************************************/
void CNCLDDSlider::OnMouseMove(UINT nFlags, CPoint point) 
{
	if(m_TimerID > 0)
	{
...
*/
				...
				itext2 = tempstr;
				iText = "Move CNCLDDSlider " + itext2 + iText;
	...
	CNCLSliderCtrl::OnMouseMove(nFlags, point);
	ClientToScreen(&point);
	((CNCLDDform*)m_parent)->ScreenToClient(&point);
...
c
**********************************************************************/
void CNCLDDSlider::SetActive(int active)
{
...
c
**********************************************************************/
void CNCLDDSlider::DrawGuides(CPoint org_pt, POINT ptCursor)
{
...
c***********************************************************************
*/
void CNCLDDSlider::set_color(const COLORREF bcolor, const COLORREF fcolor)
{
	m_fg = fcolor;
	m_bg = bcolor; 
} 
...
**
**********************************************************************/
void CNCLDDSlider::OnDragDropCallback(CPoint pt, char *input_text)
{
...
**
**********************************************************************/
void CNCLDDSlider::SetItemNo(int itemno)
{
	m_itemno = itemno;
	if (m_prop_dlg==NULL)
	{
...
/***********************************************************************
**
**   FUNCTION: :set_prop_values(int active, char *color, char *pcolor)
**
...
**
**********************************************************************/
void CNCLDDSlider::set_prop_values(int active, char *color, char *pcolor, 
		int rflag, UD_DASIN range[2], int justified)
{
	...
	m_prop_dlg->m_justified = justified;
}

void CNCLDDSlider::set_prop_HSPvalues(int n_picarea, UD_PICAREA *picarea)
{
	int k;
