/************************************************************************
**		FILE NAME: CNCLDDListbox.cpp
**
**	 Description - Functions and implementations for
**		CNCLDDListBox class (NCL listbox)
**		with Drag&Drop
**	 CONTAINS:
**		member function of CNCLDDListBox
**
**    COPYRIGHT 2013 (c) NCCS.  All Rights Reserved.
**    MODULE NAME AND RELEASE LEVEL 
**			wsntDDListBox.cpp , 25.3
...
IMPLEMENT_DYNAMIC(CNCLDDListBox, CListBox)

/***********************************************************************
**   FUNCTION: CNCLDDListBox
**		Constructor of class CNCLDDListBox
**
**   INPUT:  none
**
**   OUTPUT :   None
...
**
**********************************************************************/
CNCLDDListBox::CNCLDDListBox(int type, int flag)
	:	m_TargetDrop(NULL),
		m_TimerID(0)
{
...
**
**   FUNCTION: ~CNCLDDListBox
**              Destructor of class CNCLDDListBox, free space.
**
**   INPUT:  None
**
**   OUTPUT :   None
...
BEGIN_MESSAGE_MAP(CNCLDDListBox, CListBox)
	//{{AFX_MSG_MAP(CNCLDDListBox)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_RBUTTONUP()
	ON_WM_MOUSEMOVE()
...
**
**********************************************************************/
int CNCLDDListBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
c
**********************************************************************/
void CNCLDDListBox::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_StartPoint = 	point;
...
c
**********************************************************************/
void CNCLDDListBox::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_StartPoint.x = -100;
	m_StartPoint.y = -100;
	if(m_TimerID)
...
c
**********************************************************************/
void CNCLDDListBox::OnRButtonUp(UINT nFlags, CPoint point) 
{
	if (m_parent==NULL)
		return;
	CMenu pmenu;
...
void CNCLDDListBox::SetProperty(CNCLFormProp *prop_dlg)
{
	RECT rectClient;
	GetWindowRect(&rectClient);	
	m_parent->ScreenToClient(&rectClient);
...
**
**********************************************************************/
void CNCLDDListBox::OnSize(UINT nType, int cx, int cy) 
{
...
**
**********************************************************************/
void CNCLDDListBox::OnMove(int x, int y)
{
	CListBox::OnMove(x, y);
	CRect rectClient;
	GetWindowRect(&rectClient);	
...
c
**********************************************************************/
void CNCLDDListBox::OnMouseMove(UINT nFlags, CPoint point) 
{
	if(m_TimerID > 0)
	{
...
/*
				...
				if (m_type==0)
					...
					iText = "Move CNCLDDListBox" + iText;
*/
//				int grid = ((CNCLDDform*)m_parent)->GetGrid
				CRect rectClient;
				GetWindowRect(&rectClient);
				((CNCLDDform*)m_parent)->ScreenToClient(&rectClient);
...
c
**********************************************************************/
void CNCLDDListBox::SetActive(int active)
{
...
c
**********************************************************************/
void CNCLDDListBox::DrawGuides(CPoint org_pt, POINT ptCursor)
{
...
**
**********************************************************************/
void CNCLDDListBox::OnTimer(UINT_PTR nIDEvent) 
{	
/*	
...
**
**********************************************************************/
void CNCLDDListBox::OnDragDropCallback(CPoint pt, char *input_text)
{
	if (m_parent!=NULL)
		return;
...
/*
	...
	if (strcmp(tok, "Move")==0)
	{
		tok = strtok(NULL, " \t\r\n");
		int display = 0;
		if (stricmp(tok, "CNCLDDListBox")!=0)
		{
			return;
		}
	}
...
**
**********************************************************************/
void CNCLDDListBox::SetItemNo(int itemno)
{
	m_itemno = itemno;
	if (m_prop_dlg==NULL)
	{
...
**
**********************************************************************/
void CNCLDDListBox::set_prop_values(double font_scale, int active, char *color, char *pcolor)
{
...
void CNCLDDListBox::set_prop_HSPvalues(int n_picarea, UD_PICAREA *picarea)
{
	int k;
