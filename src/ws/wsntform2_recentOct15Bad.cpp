...
extern "C" UD_METHOD UD_initfrm_intry, UD_form_secsel;	
...
extern "C" void uw_play_videofile(char *vfile);
...
//test		((CMFCButton*)pChildWnd)->SetImage(IDB_BMP1);
	}
	windowRect2.right = windowRect2.left - 5;
	windowRect2.left = windowRect2.right - butlen;
...
/*
.....if video button, play video
*/
	if (m_type[fldno]==25)
	{
/*
.....if user not setup the video file, play default file in the form file
*/
		if (m_fData.ud_data[fldno].ud_delem.frmstr==UU_NULL)
		{
/*
.....add path to video file and play
*/
			strcpy(vfile, m_fStruct.input_flds[fldno].vfile);
/*
		status = ul_open_mod_file("UU_USER_SETTINGS", "forms", "UD_FORMDIR", (char*)UU_NULL, 
					vfile, 0, (FILE**)UU_NULL);
*/
/*
......open Video file from Video path instead of form path
*/
			status = ul_open_mod_file("UU_USER_SETTINGS", "videos", "UL_NCL_VIDEO_PATH", (char*)UU_NULL, 
					vfile, 0, (FILE**)UU_NULL);
			if (status==0)
			{
				uw_play_videofile(vfile);
			}
			else
			{
/*
...
*/
				if (UD_novideo_error==0)
				{
					sprintf(msg, "Video file:  %s  not found!", vfile);
					ud_wrerr(msg);
				}
			}
		}
		...
		{
			if (m_fData.ud_data[fldno].ud_delem.frmstr[0]!='\0')
			{
/*
.....play user input video file
*/
				strcpy(vfile, m_fData.ud_data[fldno].ud_delem.frmstr);
				status = ul_open_mod_file("UU_USER_SETTINGS", "videos", "UL_NCL_VIDEO_PATH", (char*)UU_NULL, 
						vfile, 0, (FILE**)UU_NULL);
				if (status==0)
				{
...
{
	UINT id = (UINT)wparm;
	int i, flag, stat, redisp, jmpflag;
	int fldno = -1;
/*
......only if form exist
*/
...
/**********************************************************************
**    I_FUNCTION :  chk_field(int fldno, int flag, UD_FSTAT fstat)
...
/*
......allow empty input for all input, not give error message
*/
	if (lstr[0]=='\0')
	{
		if (m_type[fldno] ==3)
...
/**********************************************************************
**    I_FUNCTION :  form_ckdata(fno,buf, flag)
...
/**********************************************************************
**    I_FUNCTION :  Get_field_data(fieldno, data, strflag)
...
*/
		...
		case 18:
			...
			break;
		case 25:
			init_button4(i, fid, bcolor, m_fStruct.input_flds[i].prompt);
			break;
		case 19:
			init_listctl2(i, fid);
			cwin = (CWnd*)(m_pScrollView->GetDlgItem(fid));
...
void CNCLForm::init_button4(int i, UINT fid, COLORREF bcolor, char *imgfile)
{
	if (UD_dispfrm[m_frmid]==NULL) return;
	CRect rbtn;
	...
	cwin->GetWindowRect(&rbtn);
	cwin->ShowWindow(SW_HIDE);
	m_pScrollView->Recreate_imgbutton(i, imgfile, 
			WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON | BS_CENTER | WS_TABSTOP | BS_BITMAP | BS_NOTIFY,
			rbtn, fid, bcolor);	
}
...
/**********************************************************************
**    I_FUNCTION :  Adjust_pos2(CRect &rect, x,y,cx,cy, att_win, ref_flag)
**       Adjust form position according to the form structure position definition. 
**    PARAMETERS   
**       INPUT  : 
**          rect : Window old postion and size
...
/**********************************************************************
**    I_FUNCTION : SetSecColor(char *name, int color, int bold)
...
/*
.....add section button callback from function call
*/
	if (UD_form_secsel!=NULL)
	{
		tmpdata.frmint = &page;
		(*UD_form_secsel)(&m_frmid, &tmpdata, UD_TFWD);
	}
...
/***********************************************************************
c
c   FUNCTION: uw_ntform_set_picarea(int frmid, int flag)
c
c       Set the form pocket flag
c
c   INPUT:  frmid: form id
...
/*********************************************************************
**    E_FUNCTION     : nclu_play_videofile(vfile)
**       play a video file
**    PARAMETERS   
**       INPUT  : 
**          none.
...
/*
	char *ptr = ux_getenv("UL_NCL_VIDEO",UX_NPRTERRS);
	if (ptr!=NULL)
	{
		syswin = -1;
		sprintf_s (command, UX_MAX_PATH_LEN+40, "cmd.exe /c \"start %s \"%s\"\"", ptr, vfile);
	}
	else
	{
		...
		if ((major>=6)&&(minor>1))
			...
			syswin = 0;
		if (syswin==0)
			sprintf_s (command, UX_MAX_PATH_LEN+40, "cmd.exe /c \"start wmplayer \"%s\"\"", vfile);
		else
			sprintf_s (command, UX_MAX_PATH_LEN+40, "cmd.exe /c \"start \"Movies & TV\" \"%s\"\"", vfile);
	}
*/
	sprintf_s (command, UX_MAX_PATH_LEN+40, "cmd.exe /c \"start %s\"", vfile);
	int result = CreateProcess(NULL, command, NULL, NULL, FALSE, 
		NULL, NULL, NULL, &stinfo, &info);	if (result==0)
	{
