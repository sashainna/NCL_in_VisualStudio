...
*/
	...
	if (status!=-1)
	{
		...
		if ((*fieldno>=LEVELRG1)&&(*fieldno<=LEVELRG6))
			Sacc[Levels] = 1;
		if ((*fieldno==OPTRG2)||(*fieldno==OPTRG2)||(*fieldno==OPTRG3))
			Sacc[Options] = 1;
		S_enable_buttons();
	}
/*
...
UD_FSTAT stat;
{
	...
	case OPTRG3:
		...
		if (exte!=NULL)
		{
			...
			{
				ud_set_traverse_mask(OPTRG3,UU_TRUE);
			}
			if (Stog_ch[2] != Taxis5)
			{
				if (Spreviewflg) Schgmade = 1;
