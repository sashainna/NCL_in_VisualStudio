...
**		 void		sfplio1(key1,key2,irpt,ptx,np,pts,ier)
...
/*********************************************************************
**    E_FUNCTION   : void ncl_tripln_io(tri1,pt,nvec,tol,eps,lSort)
...
UU_LOGICAL lSort;
{
	...
	else
	{
		...
		{
			if (d23 > tol) return;
			if ((tri1->p2[0]!=tri1->p2[0])||(tri1->p2[1]!=tri1->p2[1])||(tri1->p2[2]!=tri1->p2[2])||(tri1->p3[0]!=tri1->p3[0])||(tri1->p3[1]!=tri1->p3[1])||(tri1->p3[2]!=tri1->p3[2]))
					return;
			...
			ad1+ad2+ad3 > 100.*tol && d > 0.04*eps))
	{
		...
		{			
			lfirstime = UU_TRUE;
			if ((npt1[0]!=npt1[0])||(npt1[1]!=npt1[1])||(npt1[2]!=npt1[2])||(npt2[0]!=npt2[0])||(npt2[1]!=npt2[1])||(npt2[2]!=npt2[2]))
					return;
...
/*
.....Added for PMILL - ASF 3/3/14.
*/
		um_vcmnvc(tri1->p2,tri1->p1,v1);
		um_vcmnvc(tri1->p3,tri1->p1,v2);
 		um_cross(v1,v2,segm->norm);
		um_unitvc(segm->norm,segm->norm);
...
void ncl_tripln_io01(tri1,pt,nvec,tol,eps,lSort, cvpt)
UM_trian *tri1;
UM_coord pt, cvpt;
UM_vector nvec;
UU_REAL tol,eps;
...
*/
		...
		um_middlept(segm->pt1,segm->pt2,cvpt);
...
void ncl_tripln_io1(tri1,pln,tol,eps, cvpt)
struct NCL_nclpl_rec *pln;
UM_trian *tri1;
UU_REAL tol,eps;
UM_coord cvpt;
{
	if ((tri1->p2[0]!=tri1->p2[0])||(tri1->p2[1]!=tri1->p2[1])||(tri1->p2[2]!=tri1->p2[2])||(tri1->p3[0]!=tri1->p3[0])||(tri1->p3[1]!=tri1->p3[1])||(tri1->p3[2]!=tri1->p3[2]))
					return;
	ncl_tripln_io01(tri1,pln->pt,pln->nvec,tol,eps,UU_FALSE, cvpt);
}
