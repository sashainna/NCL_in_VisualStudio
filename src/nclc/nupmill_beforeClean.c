...
UD_FSTAT stat;
{
	...
	{
		uval=atof(Tdirval1);
		//uw_ntdispmsg("PL1 creating 0");
		if ((uval<=0.0)|| (uval >=1))
		{
			uw_ntdispmsg("Value should be selected 0 < u < 1");
			return(UD_BADACT);
		}
		//uw_ntdispmsg("PL1 creating 1");
		status = key_of_label("PT1", status);
		//uw_ntdispmsg("PL1 creating 2");
		if (status<=0)
		{
			uw_ntdispmsg("No port surfave exists or selected");
			return(UD_BADACT);
	...
	else if (*fieldno == MOTIONTRG11)
	{
		...
		{
			uw_ntdispmsg("First plane not selected");
			return(UD_BADACT);
		}
		status = nclu_pt_from_crv("PT3");
		status = nclu_pt_on_crv_start("PT3","CV4");
...
UU_LOGICAL flg;
{
	...
	//ncl_add_token(&cmdbuf, "SPIR", NCL_nocomma);
...
char* curve_name;
{
	...
	int status;
		ncl_init_cmdbuf(&cmdbuf);
		//uw_ntdispmsg("C3 creatin 0");
		nclu_pt_from_crv(point_name);
		//uw_ntdispmsg("C3 creatin 1");
...
UU_KEY_ID key;
{
	...
	//uw_ntdispmsg("getting key 0");
	//for (i=nc;i<64;i++) label[i] = " ";
	//uw_ntdispmsg("getting key 1");
	getkey(UM_addr_of_f77_str(f77_str),&key);
	//uw_ntdispmsg("getting key 2");
	done:;
	return (key);
}
