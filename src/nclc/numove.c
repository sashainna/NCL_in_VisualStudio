/*********************************************************************
**    NAME:  numove.c.
**       CONTAINS: NCL move operations.
**
**       int ncli_move
**       int nclu_move_translate
**       int nclu_move_rotate
**       int nclu_move_scale
**       int nclu_move_mirror
**       int nclu_move_matrix
**
...
/*********************************************************************
**    E_FUNCTION: ncli_move(cmdbuf, mx_name)
**       For each entity in the current select buffer, call NCL to
**       move the entity by the specified matrix. (mx_name)
**    PARAMETERS   
**       INPUT  : 
**          cmdbuf         buffer to put move statements in 
**          mx_name        name of matrix to be used by move statements
**       OUTPUT :  
**          cmdbuf         buffer move statements are loaded in
**    RETURNS: none
**    SIDE EFFECTS : none
**    WARNINGS     : none
*********************************************************************/
ncli_move(cmdbuf, mx_name)
   NCL_cmdbuf *cmdbuf;
   char *mx_name[];
   ...
   int status;
/*
.....A flag to insure that the user isn't trying to move a matrix.
*/
   int matrixflag=0;

   uu_denter(UU_MTRC,(us,"ncli_move()"));
         ...
         if (ncl_legal_relation(e1.rel_num))
           {
	            ...
	            ncl_init_cmdbuf(cmdbuf);
  	            ...
  	            status = ncl_add_token(cmdbuf, NCL_move, NCL_nocomma);
...
/*
.....If matrixflag is still 0 then the user was trying to move 
...
/*********************************************************************
**    E_FUNCTION: nclu_move_translate(option)
...
/**UM_coord from;
   ...
   uu_denter(UU_MTRC,(us,"nclu_move_translate()"));
...
/*********************************************************************
**    E_FUNCTION: nclu_move_rotate(option)
...
/**UM_coord rotpt;**/  
   ...
   uu_denter(UU_MTRC,(us,"nclu_move_rotate(%d)",option));
...
/*    created a new mask for move and clone. kathy */
   ...
   ud_lgeo(UU_TRUE, UD_ncl_mvclngeom);
   ud_ldas(UD_DASSELECT, /*pick entities to rotate: */UM_MODEL, 74,
         UU_NULL, 0, &numint,UD_NODEFAULT);
   if (numint <= 0) goto done;
/*
..... aak 15-apr-1998: check if among the selected entities there is
...
/**UM_coord mirrpt;**/
   ...
   uu_denter(UU_MTRC,(us,"nclu_move_mirror()"));
...
nclu_move_matrix()
   ...
   uu_denter(UU_MTRC,(us,"nclu_move_matrix()"));
...
/*    created a new mask for move and clone. kathy */
   ...
   ud_lgeo(UU_TRUE, UD_ncl_mvclngeom);
   ud_ldas(UD_DASSELECT, /*pick entities to move:*/UM_MODEL, 71, 
            UU_NULL, 0, &numint,UD_NODEFAULT);
   if (numint <= 0) goto done;
/*
..... aak 15-apr-1998: check if among the selected entities there is
...
*/
   ...
   if (status == NCL_OKINPUT) ncli_move(&cmdbuf, mx_name);
