...
public:
	...
	void getcmdtext(CString &label, CString &text, int &line, int &upload);
	void setcmdtext(CString label, CString text, int line = 0, int upload = 1);
	void getcmdtext2(CString &text, int &line);
	HWND getcmdwin();
	void insert_cmdstr(CString string);
	void replace_cmdstr(char *string);
	void setcmdsel(int start, int end);
	void setcmdlsel(int start, int end);
	void setcmdfocus(int focus);
	void get_ecurpos(int *start, int *end);
	void get_lcurpos(int *start, int *end, int*ln);
	...
	void set_cmd_lnend();
	void ActLine();
	void GetSingleRect(RECT &rect);
	void reshow_edit();
	void SetTFont();
	void Deset_insert();
	void set_insline(int line);
	void handle_insln();
