...
UU_LIST sins, eins;
static UU_LOGICAL Winit[NCVS]={UU_FALSE};
...
/*********************************************************************
**    E_FUNCTION     : ncl_profile_project (which,dis,tlaxfl,npts,maxan, ier)
...
**          maxan    = Max angle between tilted begin, end points
**       OUTPUT :
**          npts     = Number of points generated.  0 means error.
...
**    WARNINGS     : none
*********************************************************************/
void ncl_profile_project(which,dis,tlaxfl,npts,maxan,ier)
UM_int4 *which;
UM_int4 *npts;
UM_int4 *tlaxfl;
UM_int2 *ier;
...
*/
	*npts = 0;
	...
	{
		uu_list_init(&sins,sizeof(int),nc+10,25);
	}
	if (eins.max_cnt>0)
	{
		UU_LIST_EMPTY(&eins);
	...
	{
		uu_list_init(&eins,sizeof(int),nc+10,25);
	}
...
*/	
			...
			{
				S_calc_tilt_angles(PJPT,&nc, maxan);
...
{
	int i,nc,nent,iw,np,icm, inco, laco, shift;
	UM_int2 ids,ient,itype;
	UM_int4 twhich;
	UU_REAL wdis[6],pts[6],ptpd[6];
	UM_coord *pp,pte,*pp1, first_point_inserted, last_point_inserted, tfirst_point_inserted, tlast_point_inserted, pps;
	UM_vector *pv,pvs,pve,*pv1, first_vec_inserted, last_vec_inserted, tfirst_vec_inserted, tlast_vec_inserted;
	UU_LIST *lspt,*lsvc,mypt,myvc;
	UU_LIST ins_point_first; 
	UU_LIST ins_point_last;
	UU_LIST ins_vec_first; 
	UU_LIST ins_vec_last;
	UU_LIST first_indices;
	UU_LIST last_indices;

	UU_REAL len,mlen,rdis,tang,rper,dis[2],ang[2],dis0, dis1;

	uu_list_init(&ins_point_first,sizeof(UM_coord),0,25);
	uu_list_init(&ins_point_last,sizeof(UM_coord),0,25);
	uu_list_init(&ins_vec_first,sizeof(UM_vector),0,25);
	uu_list_init(&ins_vec_last,sizeof(UM_vector),0,25);

	uu_list_init(&first_indices,sizeof(int),0,25);
	uu_list_init(&last_indices,sizeof(int),0,25);

	uu_list_push_list(&first_indices,&sins);
	uu_list_push_list(&last_indices,&eins);
/*
...
*/
		...
		while(first_indices.cur_cnt>0)
		{
			uu_list_pop(&first_indices, &inco);

			if((inco > 0) && (inco < nc-1))
			{
				um_vctovc(pp[inco],first_point_inserted); 
				um_vctovc(pv[inco],first_vec_inserted);
				uu_list_push(&ins_point_first, &first_point_inserted);
				uu_list_push(&ins_vec_first, &first_vec_inserted);
			}

			// exclude the above points and vectors
			uu_list_delete(&Scvpt[*which],inco,1); 
			nc--;
			shift++;

			uu_list_delete(&Scvvc[*which],inco,1); 
		}


		while(last_indices.cur_cnt>0)
		{
			uu_list_pop(&last_indices, &laco);
			laco-=shift;
			if((laco > 0) && (laco < nc))
			{
				um_vctovc(pp[laco],last_point_inserted); 
				um_vctovc(pv[laco],last_vec_inserted);
				uu_list_push(&ins_point_last, &last_point_inserted);
				uu_list_push(&ins_vec_last, &last_vec_inserted);
			}
			// exclude the above points and vectors
			uu_list_delete(&Scvpt[*which],laco,1);
			nc--;
			uu_list_delete(&Scvvc[*which],laco,1);
		}
		...
		*npts = Snpts[WDPT];
		...
		// The block to insert the points back
		ang[0] = Sps_tilt_ang[0]/UM_RADIAN; 
		ang[1] = Sps_tilt_ang[1]/UM_RADIAN;


		if (ang[0] != 0.)
		{
			shift = 0;
				for (i=shift;(i<*npts) && (ins_point_first.cur_cnt>0);/*i+=shift*/)
				{
					shift = 0;
					uu_list_pop(&ins_point_first, &first_point_inserted);
					uu_list_pop(&ins_vec_first, &first_vec_inserted);

					
					um_vctovc(pp[i],pps);
					mlen = um_dcccc(pp[i],first_point_inserted);
					dis0 = um_dcccc(pp[i],pp[i+1]);
			more:	if (mlen < dis0-Stol)
					{
						if (i==0)
						{
							uu_list_insert(&Scvvc[WDPT],i+1,first_vec_inserted); 
							uu_list_insert(&Scvpt[WDPT],i+1,first_point_inserted);
						}
						else
						{
							uu_list_insert(&Scvvc[WDPT],i,first_vec_inserted); 
							uu_list_insert(&Scvpt[WDPT],i,first_point_inserted);
						}
							S_list_ary(WDPT,&pp,&pv,&nc);
							Snpts[WDPT]++;
						shift++;
		...
		*npts = Snpts[WDPT];
		...
		if (ang[1] != 0.)
		{
				for (i=*npts-1; (i>0) && (ins_point_last.cur_cnt>0);/*i--*/)
				{
					shift = 0;
					uu_list_pop(&ins_point_last, &last_point_inserted);
					uu_list_pop(&ins_vec_last, &last_vec_inserted);
					um_vctovc(pp[i],pps);

				
					mlen = um_dcccc(pp[i],last_point_inserted);
					dis1 = um_dcccc(pp[i],pp[i-1]);
more1:				if (mlen < dis1-Stol) 
					{
							uu_list_insert(&Scvvc[WDPT],i,last_vec_inserted); 
							uu_list_insert(&Scvpt[WDPT],i,last_point_inserted);
...
/*
.....Make sure first & last points weren't deleted. Not mine, Sasha
*/
...
*/
		if (Snpts[WDPT] > 0)
		{
			...
			if (overlap[0] != 0.)
			{
				itype = 1;
				wdis[2] = overlap[0];
				ncl_profile_entry(&iw,&itype,&ient,&ids,&icm,wdis,ptpd,pts,&nent);
				if (nent != 0)
				{
			...
			if (overlap[1] != 0.)
			{
				itype = 2;
				wdis[2] = overlap[1];
				ncl_profile_entry(&iw,&itype,&ient,&ids,&icm,wdis,ptpd,pts,&np);
				if (np != 0)
				{
					if (overlap[1] > 0.)
					{
...
/*********************************************************************
**    I_FUNCTION     : S_calc_tilt_angles(which,npt,maxang)
...
**			maxang	 =	max angle between points in tilted begin and end points
**       OUTPUT :
...
**    WARNINGS     : none
*********************************************************************/
static void S_calc_tilt_angles(which,npt,maxang)
int which,*npt;
UU_REAL* maxang;
{
	int nc,nc1,i,nind, mad,ico,k, newk, place;
	UU_REAL len,mlen,rdis,tang,rper,dis[2],ang[2],maxa, ttang;
	UM_coord *pp,pt,pt0,pth;
	...
	if (ang[0] != 0.)
	{
		...
		if (len-Stol > dis[0])
		{
			...
			if (um_mag(vc0) > UM_FUZZ)
			{
				...
				for (k=0;k<i;k++)
				{
					ttang = um_angle(pv[k+1],pv[k]);

					mad = ceil(ttang/(maxa));
					if(mad>1)	// If maxangle less than angle between two any	points before the inserted point at tilt distance	
					{
						um_vcmnvc(pp[k+1+place],pp[k+place],vc1); 
						rdis = um_mag(vc1)/mad;
						um_unitvc(vc1,vc1);
						um_vctovc(vc1,vc0);
						ttang = ttang/mad;
						
						for(ico=1; ico<mad; ico++)
						{
											
							um_translate_point(pp[k+place],rdis*ico,vc0,pt);
							uu_list_insert((char *)&Scvpt[which],k+1+place,pt);
							newk = k+1+place;
							
							uu_list_push(&sins,&newk);
										// indices of inserted points
										// used later in the weed function to temporarily
										// exclude inserted points from weeding
										// Sasha, Oct.26-Nov.12, 2017
							nind++;
							nc++;
							um_cross(vc0,pv[k+place],vc1); 
							um_unitvc(vc1,vc1);
							um_vctovc(pv[k+place],vc2);
							ttang = ttang;	// * ico;
							um_rotatept(vc2,vc1,pt0,-ttang*ico,UU_TRUE,&mx);
							uu_list_insert((char *)&Scvvc[which],k+1+place,vc2);
							place++;
						}
				...
				uu_list_insert((char *)&Scvpt[which],nind,pth);
				S_list_ary(which,&pp,&pv,&nc1);

				nc++;
				if (nind != nc-1) 
					um_vcmnvc(pp[nind],pp[nind-1],vc0); um_unitvc(vc0,vc0);

				if (um_mag(vc0) < UM_FUZZ) 
					um_vctovc(lastvc,vc0);
				um_vctovc(vc0,lastvc);
				tang = um_angle(pv[nind],pv[nind+1]) * rper;

				um_cross(vc0,pv[nind],vc1); um_unitvc(vc1,vc1);
				um_vctovc(pv[nind],vc2);
				um_rotatept(vc2,vc1,pt0,-tang,UU_TRUE,&mx);
				uu_list_insert(&Scvvc[which],nind,vc2);

				uu_list_push(&sins,&nind);
				...
				if (um_mag(vc0) > UM_FUZZ)
				{
					...
					for (k=nc1;k>i;k--)
					{
						...
						if(mad>1)	// If maxangle less than angle between two any	points before the inserted point at tilt distance	
						{
							...
							{
								um_translate_point(pp[k-place],rdis*ico,vc0,pt);
								uu_list_insert((char*)&Scvpt[which],k-place-ico+1,pt);
								newk = k-place-ico+1;
								uu_list_push(&eins,&newk);
											// indices of inserted points
											// used later in the weed function to temporarily
											// exclude inserted points from weeding
											// Sasha, Oct.26-Nov.12, 2017
								...
								um_vctovc(pv[k-place],vc2);
								um_rotatept(vc2,vc1,pt0,-ttang*ico,UU_TRUE,&mx);
								uu_list_insert((char*)&Scvvc[which],k-place-ico+1,vc2);
								place++;
								nc++;
							}
						}
					...
					if (nind != 1) 
						um_vcmnvc(pp[nind-1],pp[nind],vc0); 
					um_unitvc(vc0,vc0);
					if (um_mag(vc0) < UM_FUZZ) 
						um_vctovc(lastvc,vc0);
					um_vctovc(vc0,lastvc);
					...
					um_vctovc(pv[nind],vc2);
					um_rotatept(vc2,vc1,pt0,-tang,UU_TRUE,&mx);
					uu_list_insert((char *)&Scvvc[which],nind,vc2);
					uu_list_push(&eins,&nind);
