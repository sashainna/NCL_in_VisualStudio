...
**      for the Iris 3000 series workstations written by
**      Silicon Graphics to support true double precision 
**      without the need to use different format specifiers
**      from those in other C implementations.
**
...
#include "sdoprint.h"	/* parameters & macros for doprnt */
 ...
 *	_doprnt: common code for printf, fprintf, sprintf
 *
 *	extended to do either single- or double- precision 
 * 	floating point by SGI 9/19/83. (GB).
 */
#include <stdio.h>
#include <ctype.h>
#include <varargs.h>
...
/**SGI**/
/* and the long float versions */
extern char *_d_fcvt(), *_d_ecvt();
...
/**SGI**/
	/* The value being converted, if single-precision real */
	float 	fval;

	/* The value being converted, if double-precision real */
	long float	dval;
...
/**SGI**/
			...
			if (length) dval = va_arg(args, long float);
			else fval = va_arg(args,float);

/**SGI**/
			/* Develop the mantissa */
			bp = (length)?
			 _d_ecvt(dval, min(prec + 1, MAXECVT), &decpt, &sign):
			 ecvt(fval, min(prec + 1, MAXECVT), &decpt, &sign);
...
/**SGI**/
		...
		case 'f':
			...
			/* MAS added this */
			/* force the length != 0 so that will do long
				float without having to use %lf
			*/
...
/**SGI**/
			/* Do the conversion */
			bp = (length)?
			   _d_fcvt(dval, min(prec, MAXFCVT), &decpt, &sign):
			   fcvt(fval, min(prec, MAXFCVT), &decpt, &sign);
...
/**SGI**/
			/* Do the conversion */
			bp = (length)?
			   _d_ecvt(dval, min(prec, MAXECVT), &decpt, &sign):
			   ecvt(fval, min(prec, MAXECVT), &decpt, &sign);
