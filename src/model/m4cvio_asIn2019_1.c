...
UU_LOGICAL btrm;
{
	...
	if ((Scvio.ptlist.max_cnt==0)||(Scvio.uvlist.max_cnt==0))
		um_cvio_new();
	Scvio.ncv++;
	if (!btrm)
		Scvio.nutcv++;
...
/*********************************************************************
**    E_FUNCTION   : int  um_cvio_sort_points (tol)
...
UU_REAL tol;
{
	...
	UM_coord pt1,*pts;
	UU_LOGICAL lfirst,pmillfl;
/*
.....Added PMILL, which needs normal vectors too ASF 3/4/14.
*/
	ncl_pmill_getfl(&pmillfl);
/*
...
*/
			...
			{
				if (pmillfl)
				   um_cvio_push(head->pt1,head->norm);
				else
			...
			else
			{
			  ...
			  {
				  if (pmillfl)
					  um_cvio_insert(head->pt1,head->norm,0);
				  else
			...
			for (i = 0; i < n; i++)
			{
				if (UM_SQDIS (pt1,head->pt2) > eps || (n > 2 && i == n-1))
				{
					...
					{
						if (pmillfl)
							um_cvio_push(head->pt2,head->norm);
						else
					...
					else
					{
						...
						{
							if (pmillfl)
								um_cvio_insert(head->pt2,head->norm,0);
							else
								um_cvio_insert(head->pt2,head->norm,0);
						}
					}
...
**                          nvec,dpl,plflg,plmx,plinv,u0,v0)
...
**    WARNINGS     : none
*********************************************************************/
int um_cvio_project_points (sfkey,nvec,dpl,plflg, plflag1, plmx,plinv,tol,u0,v0)
UU_KEY_ID sfkey;
UM_vector nvec;
UU_REAL dpl,tol;
UM_transf plmx,plinv;
UU_REAL *u0,*v0;
UU_LOGICAL plflg, plflag1;
{
	int i,icv,status,n1;
	UM_coord ppi,uvi,*pts;
	UM_vector sni;
	UU_REAL co;
	UM_real8 asw;
	UM_real4 u,v,dir;
...
*/
			u = (bndr.ummx[0][0] + bndr.ummx[0][1]) * 0.5; 
			v = (bndr.vmmx[0][0] + bndr.vmmx[0][1]) * 0.5;
			um_cvio_project_points(*sfkey,plvec,dpl,plflg,UU_FALSE,plmx,plinv,wtol,&u,&v);
#if 0
		if (sf.key == 1280 && qfirst)
		{
			int qn;
...
{
	int status,nc,ncv,npts,i,idx,isf,itsk,irot,istat;
	int *np;
	UM_coord *pts; 
	UM_int4 key,pflg = 0;
