...
int writeFile2 (char* value) 
{

	if (fp = fopen( "..\\IvpErrorsSel.txt", "a" )) // Open file for writing
	{
...
/*	-- system select buffer -- */

	{
		...
		if(buf_ptr < UD_Select_ptr)
		{
			...
			/*sprintf(p_buff, " (level = %d) level value\n", level);
			writeFile2 (p_buff) ;
			sprintf(p_buff, " (pent = %d) pent value\n", pent);
			writeFile2 (p_buff) ;*/
