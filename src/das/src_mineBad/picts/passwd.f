...
c               pwdcti  pwddat  pwddcv  pwddea  pwdext  pwdfil  pwdlen
c               pwdlic  pwdopt  pwdpas  pwdrd   pwdsid  pwdspc  pwdtrm
c
...
c...following codes are not for WinNT
c
C      subroutine pwdall (clprog,clmsg,kerr)
...
C VAX-SUN-SGI-CIM-IBM-OSF-HPX-DEC-DOS-END
C SGI-SUN-IBM-OSF-HPX-START
C      call pwcall (clprog,kerr)
C      if (kerr .eq. 1) go to 9000
C      if (kerr .eq. 2) go to 9100
...
c...End of routine
c
C8000 call pwdctb (msg,clmsg)
C     return
c
c...Exceeded number of users
c
C9000 kerr  = 1
C      msg   = 'Number of ' // cprog(1:nc) // ' users exceeded.'
C      go to 8000
c
c...License not loaded
c
C9100 kerr  = 2
C     msg   = cprog(1:nc) // ' license has not been loaded.'
C     go to 8000
C     end
c
C VAX-SUN-SGI-CIM-IBM-OSF-HPX-DEC-DOS-END
...
c...      USE DFPORT
      INTERFACE
         INTEGER*4 FUNCTION getenvC(keypt, valuept, len)
            !DEC$ ATTRIBUTES C, ALIAS:'_getenvC'::getenvC
         integer keypt, valuept, len
         END FUNCTION
      END INTERFACE
...
c
C WNT-START
      nc1 = getenvC(LOC(lic_key), LOC(dev), 12)
...
C WNT-START
     ...
     1       iostat=istat,shared,err=9000)
C WNT-END
c
c...Get hardware id
c
...
c
  ...
  630 continue
  640 call pwdsid (lid,ncid,kflag,kerr)
...
c***********************************************************************
c
c   SUBROUTINE:  pwdsid (cid,knc,kerr)
c
...
c...Yurong
c
      subroutine pwdsid (cid,knc,kflag, kerr)
C WNT-START
      INTERFACE
         SUBROUTINE hostid (knum, kflag)
            !DEC$ ATTRIBUTES C :: hostid
            !DEC$ ATTRIBUTES REFERENCE :: knum
            !DEC$ ATTRIBUTES REFERENCE :: kflag
            INTEGER knum
            INTEGER kflag
         END SUBROUTINE
      END INTERFACE
