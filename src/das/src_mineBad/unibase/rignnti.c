...
/*********************************************************************
**    E_FUNCTION     :  status = ur_get_next_new_tuple_index(rel_num,&tuple_indx)
...
ur_get_next_new_tuple_index(rel_num,tuple_indx)
...
UR_TUPLE_INDX 	*tuple_indx;	/* where to start search */
{
	...
	uu_denter(UU_RTRC,(us,"ur_get_next_new_tuple_index(rel=%d, start index=%d)",
				rel_num, *tuple_indx));
	status = 0;
