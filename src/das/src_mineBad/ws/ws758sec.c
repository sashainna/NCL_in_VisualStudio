...
extern	int uw_7580ind;							/* index to jpbuf */
...
extern	int uw_7580ipt;							/* index to ipbuf */
extern int pts;

/*********************************************************************
**    I_FUNCTION :  uw_7580wswind(prms)              
**       description
**    PARAMETERS   
**       INPUT  : 
**          input
...
int uw_7580wswind(prms)              /* set workstation window. */
struct { Gint op; Gws ws; Gnrect3 wswind;} *prms;
{
	Gfloat winheight,nominal;

	uu_denter(UU_GITRC,(us,"uw_7580wswind"));
	...
	uw_7580wsxform(&(*ug_gksstli.wsopen[(*prms).ws].outptr).curxform,
			&uw_7580.wsxform,(*prms).ws);

	uu_dprint(UU_GITRC,(us,"uw_7580wswind(%g %g,  %g %g)",(*prms).wswind.llf.x,
...
/*********************************************************************
**    I_FUNCTION :  int uw_7580wsvport(prms)				
**       description
**    PARAMETERS   
**       INPUT  : 
**          input
...
.....Bobby  -  8/9/91
*/
	if (strcmp(TP_size,"AV") == 0) uw_7580.isrotate = UU_TRUE;
	else uw_7580.isrotate = UU_FALSE;

/*--- since drawing has A-horz and A-vert, doesn't have to rotate here--
	if ((!uw_7580.isrotate)&&(prms->wsvport.urb.x < (10.5*2.54/100.0))&&(!P1P2_control))
		 uw_7580.isrotate = UU_TRUE;
	else
		if ((uw_7580.isrotate)&&(prms->wsvport.urb.x > (9*2.54/100.0)))
		 	uw_7580.isrotate = UU_FALSE;
---*/

	uu_dprint(UU_GITRC,(us," uw_7580wsvport(%g %g,  %g %g)",(*prms).wsvport.llf.x,
			(*prms).wsvport.llf.y,(*prms).wsvport.urb.x,(*prms).wsvport.urb.y));
	uu_dexit;
}	/* uw_7580wsvport */
...
/*********************************************************************
**    I_FUNCTION :  int uw_7580wsxform(wstran,wsxform,ws)	
**       description
**    PARAMETERS   
**       INPUT  : 
**          input
...
int uw_7580wsxform(wstran,wsxform,ws)	
...
Gws ws;
{
	...
	vp= &(*wstran).v;
	uu_denter2(UU_GITRC,(us,"uw_7580wsxfm,wp=%g,%g,%g,%g,wv=%g,%g,%g,%g",
	...
	uu_denter2(UU_GITRC,(us,
			"uw_7580wsxform. wswind=%g %g,%g %g, scale=%g, dx,dy=%g %g",
	...
	uw_7580.vpy = ur[1];
	uu_denter2(UU_GITRC,(us,"uw_7580wsxform. adjusted wsvport=%d %d %d %d",
		ll[0],ll[1],ur[0],ur[1]));
	uu_dexit;
}	/* uw_7580wsxform */
...
/*********************************************************************
**    I_FUNCTION :  uw_7580ndctodev(f,i,ws)	
**       description
**    PARAMETERS   
**       INPUT  : 
**          input
...
int uw_7580ndctodev(f,i,ws)	/* convert (fx,fy) from ndc to (ix,iy) 
											raster coords */
int i[2];
Gfloat f[2];
Gws ws;							/* workstation id */
{
	...
	char us[180];
	i[0]=uw_7580.wsxform.scale*f[0]+uw_7580.wsxform.dx+wsplotdx+.5;
	i[1]=uw_7580.wsxform.scale*f[1]+uw_7580.wsxform.dy+wsplotdy+.5;
	uu_denter2(UU_GITRC,(us,"uw_7580ndctodev(%g %g, %d %d)",
		f[0],f[1],i[0],i[1]));
	uu_dexit;
}	/* uw_7580mdctodev */
...
/*********************************************************************
**    I_FUNCTION :  uw_7580devtondc(rast,ndc,ws)		
**       description
**    PARAMETERS   
**       INPUT  : 
**          input
...
int uw_7580devtondc(rast,ndc,ws)		/* convert from raster to ndc coords */
int rast[2];
Gfloat ndc[2];
Gws ws;
{
	int y;
	char us[180];
	ndc[0]=(rast[0]-uw_7580.wsxform.dx)/uw_7580.wsxform.scale;
	ndc[1]=(rast[1]-uw_7580.wsxform.dy)/uw_7580.wsxform.scale;
	uu_denter2(UU_GITRC,(us,"uw_7580devtondc(%g %g, %d %d)",
...
int *k;						/* dev select key no */
{
	/* Put up choice prompt for device curdevno if not already up by
		...
		if in EVENT mode, and return 1.
	*/	
	uu_denter(UU_GITRC,(us,"uw_7580awatch()"));
	uu_dexit;
}
...
int *k;							/* dev select key number */
{
		...
		*/	
	uu_denter(UU_GITRC,(us,"uw_7580awtpik()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DAWAITLOC */
uw_7580awaitloc(wid,k)
Gws wid;						/* workstation id */
...
*/
	uu_denter(UU_GITRC,(us,"uw_7580awaitloc()"));
	uu_dexit;
}
...
*/
	uu_denter(UU_GITRC,(us,"uw_7580awtval()"));
	uu_dexit;
}
...
/* Put up string prompt for device number curdevno if not already up
	...
	put choice device 3 event on Q if in EVENT mode, and return 1.
*/
	uu_denter(UU_GITRC,(us,"uw_7580awtstr()"));
	uu_dexit;
}

/*-------------------------------------------------> UG_DAWAITSTROKE */
uw_7580awtstrk(prms,reply)		/* await stroke */
int prms[],reply[];
{
	/* not implemented yet */
	uu_denter(UU_GITRC,(us,"uw_7580awtstrk()"));
	uu_dexit;
}


/*------------------------------------------------> UG_DECHOOP */
uw_7580echoop(prms,reply)				/* echo */
int prms[],reply[];
{
	/* not implemented yet */
	uu_denter(UU_GITRC,(us,"uw_7580echoop()"));
	uu_dexit;
}
...
Gfloat f[2];
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580ndcdev()"));
	uu_dexit;
}

/*------------------------------------------------> UGDDEVNDC */
uw_7580devndc(rast,ndc)			/* convert from device to NDC coords */
int rast[2];
Gfloat ndc[2];
{
	/* convert rast from device to NDC coords and put in ndc. */
	uu_denter(UU_GITRC,(us,"uw_7580devndc()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DPAGOP */
uw_7580pagop()				/* page (clears the screen) */
{
	/* same as UG_DREDRAWWS. Only here for compatibility reasons */
	uu_denter(UU_GITRC,(us,"uw_7580pagop()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DSAVSCROP */
uw_7580savscrop()			/* save screen */
{
	/*  now obsolete. Will not appear in future revisions */
	uu_denter(UU_GITRC,(us,"uw_7580savscrop()"));
	uu_dexit;
	
}

/*------------------------------------------------> UG_DRESSCROP */
uw_7580rstscrop()			/* restore screen */
{
	/*  now opsolete. Will not appear in future revisions */
	uu_denter(UU_GITRC,(us,"uw_7580rstscrop()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DATEXT */
uw_7580atext(x,y,s)				/* alpha text */
int x,y;
char *s;
{
	/* put up the alpha text pointed to by s at x,y.  */
	uu_denter(UU_GITRC,(us,"uw_7580atext()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DRASGET */
uw_7580rasget(rasll,rasur,savmem,rule) /* read a raster rectangle */ 
										...
										UG_COPY UG_OR UG_XOR UG_AND UG_NOT UG_CLEAR UG_SEG */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580rasget()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DRASPUT */
uw_7580rasput(rasll,rasur,savmem,rule) 	/* put a raster rectangle */
int rasll[2];					/* lower left of rectangle, raster coords */
int rasur[2];					/* upper right of rectangle */
										...
										UG_COPY UG_OR UG_XOR UG_AND UG_NOT UG_CLEAR UG_SEG */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580rasput()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DRASCPY */
uw_7580rascpy(lls,urs,lld,rule) /* copy a raster rectangle */
										...
										UG_COPY UG_OR UG_XOR UG_AND UG_NOT UG_CLEAR UG_SEG */
{
	...
	for the copy. */
	uu_denter(UU_GITRC,(us,"uw_7580rascpy()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DRASALLOC */
uw_7580rasaloc(ws,ll,ur,pt)		/* allocate memory for a raster rectangle */
Gws ws;							/* workstation id */
...
int **pt;							/* pointer to pointer to memory */
{
	/* allocate enough memory to hold the raster rectangle. Return
		...
		UG_DRASDEALLOC */
	uu_denter(UU_GITRC,(us,"uw_7580rasaloc()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DRASDEALLOC */
uw_7580rasdaloc(ws,pt)			/* deallocate memory for a raster rect */
Gws ws;							/* workstation id */
int *pt;							/* pointer to memory */
{
	/* deallocate the memory pointed to by pt. Pt contains whatever
		was placed din it by UG_DRASALLOC. */
	uu_denter(UU_GITRC,(us,"uw_7580rasdaloc()"));
	uu_dexit;
}


/*------------------------------------------------> UG_DKBD */
uw_7580kbd(buf,k)		/* UG_DKBD table entry -- get string from kbd. */
unsigned char *buf;
int *k;
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580kbd()"));
	uu_dexit;
}

/*------------------------------------------------> UG_D1CHAR */
uw_75801char(k)		/* UG_D1CHAR entry - get one char from kbd. */
int *k;								/* returned char */
{
	/* 	Do not
	...
	uu_denter(UU_GITRC,(us,"uw_75801char()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DKEYPAD */
uw_7580keypad(n,k)			/* UG_DKEYPAD entry - get keypad key.
...
int *k;					/* which key number was struck */
{
	/*	Return values:
		...
		3=a tablet puck button was used, k contains number. 
			(this is not possible on 4115)  */
	uu_denter(UU_GITRC,(us,"uw_7580keypad()"));
	uu_dexit;
}

/*------------------------------------------------> UG_DTRK */
uw_7580trk(wid,xy,k,cursorno,locno,devno,usrc)	/* UG_DTRK entry - track a cursor */
Gws wid;						/* workstation id */
...

{
	...
	uu_denter(UU_GITRC,(us,"uw_7580trk()"));
	uu_dexit;
}

/*----------------------------------------------> UG_DPIK */
uw_7580pik(wid,seg,depth,xy,k,cursorno)
									...
									1=use menu picking cursor */
{
	...
	/* UG_DPIK workstation table entry -- track a picking cursor.
		...
		segno= -1. */
	uu_denter(UU_GITRC,(us,"uw_7580pik()"));
	uu_dexit;
}

/*-------------------------------------------> UG_DCHOICE */
uw_7580choice(devno,xy,k,choice)	/* UG_DCHOICE -- phys. "choice" device */
...
int *choice;		/* on return contains menu choice number, or zero */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580choice()"));
	uu_dexit;
}

/*-------------------------------------------> UG_DBUTTON */
uw_7580button(devno,k,n)	/* UG_DBUTTON entry -- phys. button (switch) device */
...
unsigned long *n;	/* contains the "or" of the states of all the buttons */
{
	/* Get data from a physical button device.
		...
		5= an inappropriate device was used, k contains device type */
	uu_denter(UU_GITRC,(us,"uw_7580button()"));
	uu_dexit;
}

/*--------------------------------------------> UG_DVAL */
uw_7580val(devno,k,v)	/* UG_DVAL entry -- phys. valuator device */
...
Gfloat *v;				/* contains value */
{
	/* Get data from a valuator device.
		...
		4= the expected device type was used
		5= an inappropriate device was used, k contains device type */
	uu_denter(UU_GITRC,(us,"uw_7580val()"));
	uu_dexit;
}

/*--------------------------------------------> UG_DSTREAM */
uw_7580stream(func,k,cursorno)	/* UG_DSTREAM entry -- tablet stream */
										...
										have only one. */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580stream()"));
	uu_dexit;
}
...
uw_7580movop(x,y)				/* move to x,y */
int x,y;
{
	/* no longer used. should delete */
	uu_denter(UU_GITRC,(us,"uw_7580movop()"));
	uu_dexit;
}

/*---------------------------------------------> UG_DDRWOP */
/*********************************************************************
**    I_FUNCTION :  uw_7580drwop(prms)					
...

{
	...
	uu_denter(UU_GITRC,(us, "7580drwop(%g,%g,%g,%g)",(*prms).p1.x,(*prms).p1.y,(*prms).p2.x,(*prms).p2.y)); 
	uw_7580ndctodev(&(*prms).p1,xy1,uw_7580.wid);
	uw_7580ndctodev(&(*prms).p2,xy2,uw_7580.wid);
	uwi_7580drwop1(xy1,xy2);
	uu_dexit;
}	/* uw_7580drwop */
...
/*********************************************************************
**    I_FUNCTION :  uwi_7580drwop1(xy1,xy2)
**       This routine is separated from uw_7580drwop in order to be able
...

{
	...
	uu_denter(UU_GITRC,(us,"7580drwop1(%d,%d; %d,%d)",xy1[0],xy1[1],xy2[0],xy2[1]))
	if (uw_7580.isrotate)
	  {
		tmpx = -xy1[1] + uw_7580.vpy;
		tmpy = xy1[0];
		xy1[0] = tmpx;		xy1[1] = tmpy;
		tmpx = -xy2[1] + uw_7580.vpy;
		tmpy = xy2[0];
...
.....Bobby  -  8/26/91
*/
		xy1[0] = xy1[0] - DEVYHALF_758;
		xy1[1] = xy1[1] - DEVXHALF_758;
		xy2[0] = xy2[0] - DEVYHALF_758;
		xy2[1] = xy2[1] - DEVXHALF_758;
	uu_dprint(UU_GITRC,(us, "7580drwopa,xy1=%d,%d,xy2=%d,%d",
		xy1[0],xy1[1],xy2[0],xy2[1]));
	  }
	else
	{
		xy1[0] = xy1[0] - DEVXHALF_758;
		xy1[1] = xy1[1] - DEVYHALF_758;
		xy2[0] = xy2[0] - DEVXHALF_758;
		xy2[1] = xy2[1] - DEVYHALF_758;
	}
	uu_dprint(UU_GITRC,(us, "uw_7580drwop2(%d,%d,%d,%d)",xy1[0],xy1[1],xy2[0],xy2[1]));
									/* save the information to the buffer */
	color = ug_gksstli.curprats.lnbundl.color;
	uu_dprint(UU_GITRC,(us, "758color,color=%d", color));
	if (color > 8)   color = 1;
	jpbuf[uw_7580ind].color = color;
	jpbuf[uw_7580ind].pt1[0] = xy1[0];
	jpbuf[uw_7580ind].pt1[1] = xy1[1];
	jpbuf[uw_7580ind].pt2[0] = xy2[0];
	jpbuf[uw_7580ind].pt2[1] = xy2[1];
	uw_7580ind++;

	/* added kathy */
	/*if (uw_7580ind == MAXJP)*/
	if ((uw_7580ind+uw_7580ipt) == MAXJP)
		{
		 uw_7580dmpline();
		 uw_7580ind = 0;
		}
	uu_dexit;
/* --------------------------------------------------------------*/
/*		 uu_ttput(uw_7580.ttfd,"SP ",3);			/* select pen color */
/*		 sprintf(num, "%d;", ug_gksstli.curprats.lnbundl.color);
		 uu_ttput(uw_7580.ttfd,num,strlen(num));			/* select pen color */
/*		 uu_ttflout(uw_7580.ttfd);
	uu_denter2(UU_GITRC,(us, "758color,num=%s", num));
	uu_dexit;
 	uu_ttput(uw_7580.ttfd,"PU ",3);			/* pen up and move */
/*	sprintf(num, "%d,%d; ", xy1[0], xy1[1]);
 	uu_ttput(uw_7580.ttfd,num,strlen(num));
 	uu_ttput(uw_7580.ttfd,"PD ",3);			/* pen down and draw */
...
/*
.....Added new uw_7580rasline routine which
...
int x1,y1,x2,y2;
{
	...
	if (color > 8) color = 1;
	jpbuf[uw_7580ind].color = color;
	jpbuf[uw_7580ind].pt1[0] = x1;
	jpbuf[uw_7580ind].pt1[1] = y1;
	jpbuf[uw_7580ind].pt2[0] = x2;
	jpbuf[uw_7580ind].pt2[1] = y2;
	uw_7580ind++;
	if ((uw_7580ind+uw_7580ipt) == MAXJP)
	{
		uw_7580dmpline();
...

{
	...
	uu_denter(UU_GITRC,(us, "7580dmpline,uw_7580ind=%d", uw_7580ind));
...

{
	...
	uu_denter(UU_GITRC,(us, "7580raspnts,uw_7580ipt=%d", uw_7580ipt));
...
*/
			...
			uu_ttput(uw_7580.ttfd,num,strlen(num));
/* 			  uu_ttput(uw_7580.ttfd,"SM+ ",3);			/* set the marker */
/*
.....Changed PD to PU
.....Bobby  -  8/9/91
*/
 			  uu_ttput(uw_7580.ttfd,"PU ",3);
			  sprintf(num, "%d,%d; ", ipbuf[j].pt1[0], ipbuf[j].pt1[1]);
 			  uu_ttput(uw_7580.ttfd,num,strlen(num));
...
*/
	...
	uu_denter(UU_GITRC,(us,"uw_7580pntop()"));
	uu_dexit;
	...
	uw_7580ndctodev(&(*prms).p,xy1,uw_7580.wid);
...
*/
	...
	uu_denter2(UU_GITRC,(us, "758color,color=%d", color));
	uu_dexit;
	if (color > 8)   color = 1;
	ipbuf[uw_7580ipt].color = color;
/*
.....Save marker type
.....Bobby  -  8/9/91
*/
	ipbuf[uw_7580ipt].type = mtype[(*prms).type-1];
	ipbuf[uw_7580ipt].pt1[0] = xy1[0];
	ipbuf[uw_7580ipt].pt1[1] = xy1[1];
...
/*---------------------------------------------> UG_DCHHTNDC */
uw_7580chhtndc(prms)				/* set character height NDC */
struct { Gint op; Gws id; Gfloat ndc_ht; } *prms;
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580chhtndc()"));
	uu_dexit;
}

/*---------------------------------------------> UG_DERASE */
uw_7580erase(prms)				/* set xor mode */
int prms[3];
{
	/* No longer used.  Will not appear in future revisions */
	uu_denter(UU_GITRC,(us,"uw_7580erase()"));
	uu_dexit;
}

/*----------------------------------------------> UG_DPROMPT */
uw_7580prompt(ws,prompt,loc,num)			
...
int num;								/* number of the prompt, 0<=num<=710 */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580prompt()"));
	uu_dexit;

}

/*---------------------------------------------> UG_DDNPROMPT */
uw_7580dnprmpt(ws,num)
Gws ws;
int num;								/* number of the prompt */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580dnprmpt()"));
	uu_dexit;
}

/*----------------------------------------------> UG_DMENU */
uw_7580menu(wid,deviceno,prompt)	
Gws wid;									/* workstation id */
Gchar *prompt;							/* prompt */
int deviceno;							/* choice device number of this menu */
{
	...
	uu_denter(UU_GITRC,(us,"uw_7580menu()"));
	uu_dexit;
}

/*-----------------------------------------------> UG_DDNMENU */
uw_7580dnmenu(wid,devno)			
Gws wid;						/* workstation id */
int devno;					/* choice device number */
{
	...
	/* The same comment applies here as applied for UG_DDNPROMPT above.
		...
		be used. */
	uu_denter(UU_GITRC,(us,"uw_7580dnmenu()"));
	uu_dexit;
}
