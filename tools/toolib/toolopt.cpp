...
extern "C" void ul_getvalid_fulldir(char *dir,char *fulldir);
...
static void SetDirectory(int nfld, int *ifld, char **ftext, char **fdir)
{
	...
	WCHAR wdefdirec[UX_MAX_PATH_LEN];
	int len;
	...
	for (i=0; (i<m_dirnum)&&(i<20); i++)
	{
		...
		if (tempdir[0]!='\0')
		{
			...
			if (direc[0]=='\0')
			{
				GetCurrentDirectory(UX_MAX_PATH_LEN, wdefdirec);
				len = wcslen (wdefdirec);
				wcstombs(defdirec, wdefdirec, UX_MAX_PATH_LEN);
				defdirec[len] = '\0';
...
extern "C" HRESULT CustomFileOpenDir(char* title, int nfld, int *ifld, char **ftext,
								char **fdir, char* fnam, int *nc, CWnd *MainWin)
{
    IFileDialog *pfd = NULL;
    HRESULT hr;
	hr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfd));
