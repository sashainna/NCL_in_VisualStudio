...
**    WARNINGS     : none
*********************************************************************/
extern "C" void uk_create_unibase(LPSLDWORKS pIswApp,NCLX_sw_options *swopt)
{
	int ierr;
	char lmsg[80];
	TCHAR bstr[80];
...
//
	if (hRes == S_OK)
	{
		...
		if (strlen(swopt->unibase) == 0)
		{
			...
			if (nc == 0)
			{
				pIswModelDocPart->GetTitle(&partname);
				nc = wcslen(partname);
			}
...
//
			ul_break_fname(swopt->unibase,dir,ofname);
			if (dir[0] == '\0') strcpy(dir,swopt->dir);
			ul_build_full_fname(dir,ofname,"u",swopt->unibase);
		}
//
...
void CNclUnibase::CreateUnibase(LPPARTDOC part, LPCOMPONENT2 compon)
{
	...
	double Srange[11];
	int *Siparam,ipt[5],rpt[3],inc,i,prog,*ivar,msub;
	long Ssize,count;
	VARIANT vdat;
	VARIANT_BOOL lstat;
	SAFEARRAY *psa;
...
//
				tsf.surf = &surf;
				START_TIME;
				CreateTrimSf(&tsf,Sparam,pdat,ipt,rpt,&msub);
				END_TIME(tsfc_t);
//
