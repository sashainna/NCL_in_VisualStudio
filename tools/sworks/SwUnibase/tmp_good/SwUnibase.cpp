...
#include <string>
#include <list>
#include "SwBitmapHandler.h"
#include "nclxunib.h"

extern "C" int uk_modal_config();
extern "C" void uk_create_unibase(ISldWorks **,NCLX_sw_options *);
...
/*********************************************************************
**    I_FUNCTION     :  AddUserInterface()
**       Initializes NCL/Solid and adds its user interface to SolidWorks.
**    PARAMETERS
**       INPUT  : none
**       OUTPUT : none
**    RETURNS      : none
...
**    WARNINGS     : none
*********************************************************************/
void CSwUnibase::AddUserInterface()
{
// AddToolbar();
   int ierr;
   ierr = ncl_init_auth("SOLIDWORKS");
   if (ierr == 0) AddMenus();
}
...
//	int menuToolbarOption = (int)(swMenuItem | swToolbarItem);
	int menuToolbarOption = (int)(swMenuItem);
...
//	icmdGroup->put_HasToolbar(true);
	icmdGroup->put_HasToolbar(false);
...
TCHAR tbuf[80];
int nc;
if (IsConnected == NULL) return E_POINTER;
	m_swCookie = Cookie;
	m_iSldWorks = CComQIPtr<ISldWorks, &__uuidof(ISldWorks)>(ThisSW);
/*
m_iSldWorks->get_IActiveDoc(&DocPart);
sprintf(sbuf,"m_iSldWorks = 0x%x",m_iSldWorks);
mbstowcs(tbuf,sbuf,strlen(sbuf)+1);
...
//		dlg.DoModal();
		...
		LPSLDWORKS SldWorks;
m_iSldWorks->get_IActiveDoc(&DocPart);
sprintf(sbuf,"DocPart = 0x%x",DocPart);
mbstowcs(tbuf,sbuf,strlen(sbuf)+1);
MessageBox(NULL,tbuf,_T("NCL/SOLID"),MB_OK);
		status = uk_modal_config();
		SldWorks=m_iSldWorks;
		CNclUnibase *Nuni = new CNclUnibase();
		Nuni->SaveUnibase(iSwApp,&m_swUniOptions);
//		uk_create_unibase(&iSwApp/*,SldWorks*/,&m_swUniOptions);
