...
BOOL CBitmapHandler::SaveHBitmapToDisk(CComBSTR filename, HBITMAP bitmap, HPALETTE palette = NULL)
{
	BOOL written = FALSE;
    ...
    //Create the file
    HANDLE tempFile = ::CreateFile(CString(filename), GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (tempFile)
    {
		HGLOBAL mem = NULL;
