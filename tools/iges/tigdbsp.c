...
#define UA_NV_HATCHLINE 1	/* # of varlists in UA_hatchline_rec */
#define UA_LS_HATCHLINE 200 /* initial varlist size */

UU_KEY_ID  UA_drafting_view = {0};
...
ua_sal_to_uni(e1, e2)
	...
	int i,j;
	int pts;			/*jkd39: move only points used */
	...
	for(i=0;i<e2->no_arcblk;i++)
		...
		pts = e1->arc_blk[i].num_pts; /*jkd39: move only points used */
		for(j=0;j<pts;j++)				/*jkd39 */
			{
	...
	for(i=0;i<e2->no_lineblk;i++)
		...
		pts = e1->line_blk[i].num_pts;/*jkd39: move only points used */
		for(j=0;j<pts;j++)				/*jkd39 */
			{
		...
		for(;j<50;j++)				/*MILLS: initialize unused fields -  */
			{						/* corrects reserve operand fauls on vax */
			e2->lineblk[i].line_seg[j][0] = 0.0;
			e2->lineblk[i].line_seg[j][1] = 0.0;
			e2->lineblk[i].line_seg[j][2] = 0.0;
			}
		}
