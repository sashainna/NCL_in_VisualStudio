...
extern "C" int iges_wntstr_out(char *msg, int display)
{
	...
	int len = strlen(msg);
	if (len==0) return 0;
	CIgesApp *app = (CIgesApp*)AfxGetApp();
	CIgesDlg *MainDlg = (CIgesDlg*)(app->GetMainWnd());
...
extern "C" int iges_wntget_label_type()
{
	switch (label_type)
	{
		...
		case 6:
			...
			break;
		case 7:
			uig_list_out("Labels from IGES using max 6 chars.\n\n", 0);
			break;
	}
	return(0);
}
...
*/
	...
	CIgesListDlg *listdlg = new CIgesListDlg(parent);
...
.....using drive\path\filename.igs format.
*/
	if (strlen(filename) <= 0)
	{
		...
		sprintf(path_buffer,"%s%s%s.igs", drive, dir, fname);
/*
...
extern "C" int iges_wntget_unifil(char *filename)
{
	...
	CEdit *eiwin = (CEdit*)(MainDlg->GetDlgItem(IDC_INPUT_IGES));
...
extern "C" int iges_wntprompt(CWnd *parent, char *title, char *msg, int lns, int cols, char *ans_str)
{
	...
	CIgesPromptDlg *promptdlg = new CIgesPromptDlg(parent);
...
extern "C" void iges_disply_as_percent(int num)
{
	...
	if (Iges_batch)
	{
		if (iges_process==0)
			return;
		CIgesDlg2 *MainDlg = (CIgesDlg2*)(app->GetMainWnd());
/*
