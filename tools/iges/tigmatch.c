...
/*********************************************************************
**    E_FUNCTION     :  uig_match_entity(key)
**       Compare translated entity data with existing entity data
**       to see if there is a match for labeling purposes.
**    PARAMETERS
**       INPUT  :
**          key       Key of entity to compare.
**       OUTPUT : none
...
**    WARNINGS     : none
*********************************************************************/
int uig_match_entity(key)
UU_KEY_ID key;
{
	int status;
	struct NCL_fixed_databag ent;
/*
.....Get entity to compare
*/
	ent.key = key;
...
*/
	...
	{
	case UM_POINT_REL:
		return(uig_match_point(&ent,0));
	case UM_LINE_REL:
		return(uig_match_line(&ent,0));
	case UM_CIRCLE_REL:
		return(uig_match_circle(&ent,0));
	case NCL_PLN_REL:
		return(uig_match_plane(&ent,0));
	case UM_CONIC_REL:
		return(uig_match_conic(&ent,0));
	case UM_POLYLINE_REL:
		return(uig_match_polyline(&ent,0));
	case NCL_POINTVEC_REL:
		return(uig_match_pointvec(&ent,0));
	case UM_RBSPLCRV_REL:
		return(uig_match_rbsplcrv(&ent,0));
	case UM_COMPCRV_REL:
		return(uig_match_compcrv(&ent,0));
	case UM_RBSPLSRF_REL:
		return(uig_match_rbsplsrf(&ent,0));
	case NCL_TRIMSF_REL:
		return(uig_match_trimsrf(&ent,0));
	case NCL_REVSURF_REL:
		return(uig_match_revsf(&ent,0));
	}
	return(UU_FAILURE);
}
...
/*********************************************************************
**    E_FUNCTION     :  uig_match_point(eptr,labflg)
...
*/
		if (check.label[0] == '@')
			status = UU_FAILURE;
		else if (!labflg && eptr->key == check.key)
			status = UU_FAILURE;
/*
...
*/
		if (check.label[0] == '@' || eptr->key == check.key)
			status = UU_FAILURE;
/*
...
/*
.....change for handle failure, if failed, it won't
.....have boundary_box1, will cause error later when
.....try to use this value 
.....Yurong
*/
	status = um_pre_srf_bndr_box(eptr,&boundary_box1);
	if (status != UU_SUCCESS)
		goto done;
